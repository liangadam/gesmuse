...
class My_eye(object):
    ...
    def __init__(self, px=266, py=631, raw=256, col=256):
        self.px = px
        self.py = py
        self.raw = raw
        self.col = col
        ...
        self.voi2 = np.zeros((raw, col, 3), np.uint8)
    ...
    def frame_show(self):
        ...
        self.voi2 = self.frame[PD:self.col + PD, 0:self.raw].copy().astype(np.int32)
        return 1
    ...
    def img_get(self):
        ...
        # 做差
        sub = np.abs(voi - self.voi2).astype(np.uint8)
        # 灰度化
